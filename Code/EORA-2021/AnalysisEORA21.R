...
load("Code/EAC_EORA_2021_data.RData")

# Libraries, Functions, Global Variables -----------------------------------------

library(fastverse)
fastverse_extend(ggplot2, scales, RColorBrewer, decompr)
...
va_ag_sec[, as.character(2005:2015)] %>% 
  qDT("Sector") %>% 
  ...
  scale_x_continuous(breaks = as.integer(y), expand = c(0,0.02)) +
...
lapply(decomps, with, Vc * X) %>%
  ...
  scale_x_continuous(breaks = as.integer(y)) + rbsc2 +
  theme_minimal() + pretty_plot +
  theme(legend.position = "right")

dev.copy(pdf, "Figures/EAC_GDP_sector_21.pdf", width = 11.69, height = 8.27)
dev.off()
...
GFLR %>% melt(1, value.name = "Ratio") %>% 
  ...
  scale_x_continuous(breaks = as.integer(y)) +
...
# Decomposing this effect
lapply(y, function(i) T_ag[, , i] %>% `diag<-`(0) %>% .[EAC, EAC] %>% rowSums() %>% 
         qDF(TRUE) %>% set_names(c("Country", "Value"))) %>% 
  unlist2d("Time", DT = TRUE) %>% 
  ggplot(aes(x = Time, y = Value)) + geom_line() + facet_wrap( ~ Country)
...
decomps$`2015`$Vc[eac] %>% replace_outliers(0, NA, "min") %>%
  qDF(TRUE) %>%
  setNames(.c(Sector, DVA)) %>%
  tfm(Country = mfac(cr(Sector)), Sector = mfac(crm(Sector))) %>%
  {
...
barplot(crm(decomps$`2015`$E[eac[, "UGA"]]), col = "orange", border = NA)
...
dev.copy(pdf, "Figures/exports_stacked_18.pdf", width = 8.27, height = 7.5)
dev.off()
...
dev.copy(pdf, "Figures/exports_stacked_ts_21.pdf", width = 11.69, height = 8.27)
dev.off()


# EAC Exports to EAC Partner States
decomps$`2018`$ESR[eac, EAC] %>%
  rowSums() %>%
  qDF(TRUE) %>%
...
dev.copy(pdf, "Figures/exports_EAC_18.pdf", width = 10, height = 8.27)
dev.off()

# Exports to EAC Partner States: Stacked
decomps$`2021`$ESR[eac, EAC] %>%
  rowSums() %>%
  qDF(TRUE) %>%
  setNames(.c(Sector, Exports)) %>%
  tfm(Country = mfac(cr(Sector)), Sector = mfac(crm(Sector))) %>%
...
dev.copy(pdf, "Figures/exports_EAC_stacked_21.pdf", width = 8.27, height = 7.5)
dev.off()

# Exports Share to EAC Partner States: Stacked
year <- "2018"
decomps[[year]]$ESR[eac, EAC] %>%
  rowSums() %c/% fsum(rowSums(decomps[[year]]$ESR[eac, ]), cr(rownam[eac]), TRA = 1L) %>% 
  qDF(TRUE) %>%
  setNames(.c(Sector, Exports)) %>%
  tfm(Country = mfac(cr(Sector)), Sector = mfac(crm(Sector))) %>%
  ggplot(aes(x = Country, y = Exports, fill = Sector)) +
...
dev.copy(pdf, "Figures/exports_EAC_perc_stacked_18.pdf", width = 8.27, height = 7.5)
dev.off()


# Exports Share to EAC Partner States: Stacked Area
lapply(decomps[as.character(as.integer(y))], function(x) x$ESR[eac, EAC] %>% 
         rowSums %c/% 
...
dev.copy(pdf, "Figures/exports_EAC_perc_stacked_ts_21.pdf", width = 11.69, height = 8.27)
dev.off()
...
year <- "2015"
get_E_shares(T[,, year], FD[,, year])[eac, ] %>% qDT("Sector") %>%
  tfm(Country = mfac(cr(Sector)), Sector = mfac(crm(Sector))) %>%
...
# First check:
get_E_shares(T_ag[, , year], FD_ag[, , year], return.E = TRUE) %>% View
    ...
    dev.copy(pdf, "Figures/output_shares_ag_ts_21.pdf", width = 11.69, height = 8.27)
    dev.off()
...
VB_eac_ag <- VB[, eac, "2018"] %c*% 100 %>% fsum(g)
...
dev.copy(pdf, "Figures/VA_RATIOS_21.pdf", width = 8.27, height = 4)
dev.off()
...
dev.copy(pdf, "Figures/TB_INTER_21.pdf", width = 6, height = 4.5)
dev.off()

mean(IntTB$VA - IntTB$`Gross    `)
mean(IntTB$VA/IntTB$`Gross    `)

# Combined Figure (Gross and VA Flows)

Int_ALL <- cbind(Inflow_Gross = sapply(y, function(i) T_ag[, , i] %>% `diag<-`(0) %>% {sum(.[ROW, EAC])/sum(.[EAC, EAC])}),
                 Outflow_Gross = sapply(y, function(i) T_ag[, , i] %>% `diag<-`(0) %>% {sum(.[EAC, ROW])/sum(.[EAC, EAC])}),
                 Inflow_VA = sapply(y, function(i) T_VA_ag[, , i] %>% `diag<-`(0) %>% {sum(.[ROW, EAC])/sum(.[EAC, EAC])}),
                 Outflow_VA = sapply(y, function(i) T_VA_ag[, , i] %>% `diag<-`(0) %>% {sum(.[EAC, ROW])/sum(.[EAC, EAC])}),
                 TB_Gross = sapply(y, function(i) T_ag[, , i] %>% `diag<-`(0) %>% {sum(.[EAC, ROW])/sum(.[ROW, EAC])}),
                 TB_VA = sapply(y, function(i) T_VA_ag[, , i] %>% `diag<-`(0) %>% {sum(.[EAC, ROW])/sum(.[ROW, EAC])})) %>% 
  qDT("Year") %>% tfm(Year = as.integer(Year)) %>% 
  melt("Year", value.name = "Ratio", variable.factor = FALSE) %>% 
  ftransform(Type = factor(iif(endsWith(variable, "VA"), "VA   ", "Gross"), levels = c("VA   ", "Gross")), 
             Var = sub("_Gross|_VA", "", variable) %>% recode_char(TB = "Trade Balance"))

Int_ALL %>% 
  sbt(between(Year, 2005, 2015)) %>% 
  ggplot(aes(x = Year, y = Ratio, linetype = Type)) + geom_line() + 
  facet_wrap( ~ Var, scales = "free_y") +
  scale_x_continuous(breaks = 2005:2015) +
  scale_y_continuous(limits = c(0, NA), breaks = extended_breaks(10)) +
  guides(linetype = guide_legend(title = NULL)) +
  theme_minimal() + pretty_plot # + theme(legend.position = "right")

dev.copy(pdf, "Figures/ALL_RATIOS.pdf", width = 8.27, height = 4)
dev.off()

Int_ALL %>% 
  sbt(between(Year, 2005, 2015) & Type == "Gross") %>% 
  ggplot(aes(x = Year, y = Ratio)) + geom_line() + 
  facet_wrap( ~ Var, scales = "free_y") +
  scale_x_continuous(breaks = 2005:2015) +
  scale_y_continuous(limits = c(0, NA), breaks = extended_breaks(10)) +
...
EAC_VA_shares %>%
  ...
  scale_x_continuous(breaks = as.integer(y), expand = c(0,0)) + rbsc2 +
  #scale_color_brewer(palette = "Set1") +
  # guides(fill = guide_legend(title = NULL, nrow = 1)) + 
  theme_minimal() + pretty_plot + theme(legend.position = "right")

dev.copy(pdf, "Figures/VA_shares_ag_ts_area_21.pdf", width = 11.69, height = 8.27)
dev.off()

# Simle difference of VA shares
EAC_VA_shares %>%  
  sbt(Year %in% c(2005, 2015)) %>% 
  roworderv() %>% 
  gby(Share, Country) %>% 
  mtt(Max = fmax(Value),
  ...
  fungroup() %>% 
  mtt(Year = qF(Year),
      Country_new = fifelse(as.character(Country) == as.character(Share), 
                            paste0(Country, ' (', round(Diff*100, 2), '%)'), NA_character_),
      Country_new = mfac(ffirst(Country_new, Country, "replace_fill"))) %>% 
  sbt(as.character(Country) != as.character(Share)) %>%
...
dev.copy(pdf, "Figures/VA_shares_ag_ts_bar_diff.pdf", width = 11.69, height = 8.27)
dev.off()
...
merge(EAC_exports_VA, exports_EAC_VA) %>%
  ...
  facet_wrap( ~ Country, scales = "free_y") + 
  scale_y_continuous(labels = percent, breaks = extended_breaks(7)) + 
  scale_x_continuous(breaks = as.integer(y)) + ylab("EAC Share") +
  guides(color = guide_legend(title = NULL)) +
  theme_minimal() + pretty_plot 

dev.copy(pdf, "Figures/VA_EAC_shares_ts_21.pdf", width = 11.69, height = 8.27)
dev.off()
...
dev.copy(pdf, "Figures/VA_FD_exports_stacked_ts_21.pdf", width = 11.69, height = 8.27)
dev.off()
...
dev.copy(pdf, "Figures/VA_FD_EAC_shares_ts_21.pdf", width = 11.69, height = 8.27)
dev.off()
...
VS_df_ag %>% slt(-i2e) %>% 
  ...
  scale_y_continuous(labels = percent, 
                     breaks = extended_breaks(10)) + 
  scale_x_continuous(breaks = as.integer(y)) + # rbsc +
  scale_color_brewer(palette = "Set1") +
  guides(color = guide_legend(title = NULL, nrow = 1)) + 
  theme_minimal() + pretty_plot 

dev.copy(pdf, "Figures/VS_ag_ts_21.pdf", width = 11.69, height = 8.27)
dev.off()
...
KWW_ts %>% 
  ...
  # guides(fill = guide_legend(ncol = 1)) +
  # scale_y_continuous(labels = mds, breaks = extended_breaks(7)) +
  # scale_x_continuous(breaks = as.integer(y)) + rbsc2 + ylab("Value Added") +
  # theme_minimal() + pretty_plot +
  # theme(legend.position = "right")

dev.copy(pdf, "Figures/KWW_fill_ts_21.pdf", width = 11.69, height = 8.27)
dev.off()
...
dev.copy(pdf, "Figures/KWW_fill_ts_EAC_21.pdf", width = 11.69, height = 8.27)
dev.off()
...
dev.copy(pdf, "Figures/KWW_fill_sec_18.pdf", width = 11.69, height = 8.27)
dev.off()
...
dev.copy(pdf, "Figures/UP_DOWN_ag_ts_21.pdf", width = 11.69, height = 8.27)
dev.off()


KWW_ts %>% 
  sbt(Country %in% EAC & Year <= 2015) %>%
  dcast(Year + Country ~ Term) %>% 
  fcompute(Year = Year, Country = Country,
           `Upstreamness       ` = (DVA_INT + DVA_INTrex + DDC) / (DVA_FIN + DVA_INT + DVA_INTrex + RDV_FIN + RDV_INT + DDC),
           Downstreamness = FVA_FIN / (FVA_FIN + FVA_INT + FDC)) %>% # Why ??
  melt(1:2, variable.name = "Variable", value.name = "Value") %>% 
  gby(Country, Variable) %>% fdiff(10, 1, Year) %>% fmean() %>%
  fungroup %>% na_omit %>% {
...
dev.copy(pdf, "Figures/UP_DOWN_ag_growth_21.pdf", width = 8, height = 4.27)
dev.off()

# Disaggregated  ------------------------------------------------
KWW_ts_det %>% 
  sbt(Exporting_Country %in% EAC) %>%
  gby(Year, Exporting_Country, Exporting_Industry, Term) %>% 
...
dev.copy(pdf, "Figures/UP_DOWN_sec_21.pdf", width = 11.69, height = 8.27)
dev.off()
...
dev.copy(pdf, "Figures/NRCA_21.pdf", width = 11.69, height = 8.27)
dev.off()

# Dotchart
KWW_ts_det_wide %>% 
  rm_stub("Exporting_") %>% 
  sbt((Year == 2015L | Year == 2021L) & Country %in% EAC, Year:Industry, NRCA) %>% 
  tfm(Industry = factor(Industry, levels = rev(levels(Industry)))) %>% {

  ggplot(., aes(x = NRCA, y = Industry, color = qF(Year))) +
  geom_vline(xintercept = 1) + geom_point() + 
  geom_text(aes(label = fifelse(Year == 2021, signif(NRCA, 3), NA_real_)), 
            hjust = fifelse(with(., ffirst(NRCA, list(Country, Industry), 1) < flast(NRCA, list(Country, Industry), 1)), -0.2, 1.2), cex = 3, 
            colour = "#1F78B4") +
  facet_wrap( ~ Country, scales = "free_y") + 
  scale_x_continuous(trans = "log10", breaks = log_breaks(10), limits = c(0.01, 100),
                     expand = c(0, 0.02), labels = function(x) signif(x, 3)) +
  scale_color_brewer(palette = "Paired", labels = c("2015     ", "2021 (Value Indicated)")) +
  guides(color = guide_legend(title = NULL, nrow = 1)) + 
  theme_minimal() + pretty_plot + 
  theme(axis.text.x = element_text(angle = 0, hjust = 0.5, margin = margin(t = 3.5)), 
  ...
  dev.copy(pdf, "Figures/NRCA_fl_21.pdf", width = 11.69, height = 8.27)
  dev.off()
...
dev.copy(pdf, "Figures/NRCA_ts_21.pdf", width = 11.69, height = 8.27)
dev.off()
...
KWW_ts_det_wide %>% 
  rm_stub("Exporting_") %>% 
  sbt((Year == 2015L | Year == 2021L) & Country %in% EAC, Year:Industry, NRCA_EAC) %>% 
  tfm(Industry = factor(Industry, levels = rev(levels(Industry)))) %>% {
    
    ggplot(., aes(x = NRCA_EAC, y = Industry, color = qF(Year))) +
      geom_vline(xintercept = 1) + geom_point() + 
      geom_text(aes(label = fifelse(Year == 2021, signif(NRCA_EAC, 3), NA_real_)), 
                hjust = fifelse(with(., ffirst(NRCA_EAC, list(Country, Industry), 1) < flast(NRCA_EAC, list(Country, Industry), 1)), -0.2, 1.2), cex = 3, 
                colour = "#1F78B4") +
      facet_wrap( ~ Country, scales = "free_y") + 
      scale_x_continuous(trans = "log10", breaks = log_breaks(10), limits = c(0.01, 100),
                         expand = c(0, 0.02), labels = function(x) signif(x, 3)) +
      scale_color_brewer(palette = "Paired", labels = c("2015     ", "2021 (Value Indicated)")) +
      guides(color = guide_legend(title = NULL, nrow = 1)) + 
      theme_minimal() + pretty_plot + xlab("NRCA relative to the EAC") +
      theme(axis.text.x = element_text(angle = 0, hjust = 0.5, margin = margin(t = 3.5)), 
            plot.margin = margin(r = 10))
  }

dev.copy(pdf, "Figures/NRCA_EAC_fl_21.pdf", width = 11.69, height = 8.27)
dev.off()
...
KWW_ts_det_wide %>% 
  ...
  sbt((Year == 2005L | Year == 2015L) & Country %in% EAC, Year:Industry, NRCA_EAC) %>% 
  tfm(Industry = factor(Industry, levels = rev(levels(Industry))), 
      NRCA_EAC = fgrowth(NRCA_EAC, 10, 1, list(Country, Industry), Year, power = 1/10, scale = 1)) %>% 
  {
    ggplot(., aes(y = Industry, x = NRCA_EAC)) + 
      geom_bar(stat = "identity", alpha = 0.8) +
      geom_text(aes(label = percent(NRCA_EAC, 0.1)), hjust = fifelse(.$NRCA_EAC >= 0, -0.2, 1.2), cex = 3, 
                colour = "grey10") +
      facet_wrap( ~ Country, scales = "free_y") +
      guides(fill = "none") +
      scale_x_continuous(breaks = extended_breaks(10), labels = function(x) percent(x, 1),
                         limits = c(-0.16, 0.16)) + # , expand = expansion(mult = 0.1)
      scale_y_discrete(expand = c(0, 0)) + xlab("NRCA relative to the EAC") +
      theme_minimal() + pretty_plot +
      theme(axis.text.x = element_text(angle = 0, hjust = 0.5, margin = margin(t = 3.5))) #,
  }

dev.copy(pdf, "Figures/NRCA_EAC_growth.pdf", width = 11.69, height = 8.27)
dev.off()
...
dev.copy(pdf, "Figures/NRCA_EAC_ts_21.pdf", width = 11.69, height = 8.27)
dev.off()
...
dev.copy(pdf, "Figures/NRCA_IEAC_21.pdf", width = 11.69, height = 8.27)
dev.off()
...
library(magrittr)
library(stargazer)
fdroplevels <- collapse::fdroplevels
...
dev.copy(pdf, "Figures/GROWTH_REG_TS_21.pdf", width = 10.27, height = 5)
dev.off()
...
phtest(log_VA ~ L(I2E, 0:2) + L(E2R, 0:2), to_plm(moddat))
# Robust version (same conclusion)
temp <- L(moddat, 0:2, cols = c("log_VA", "I2E", "E2R"))
phtest(log_VA ~ I2E + L1.I2E + L2.I2E + E2R + L1.E2R + L2.E2R, 
       to_plm(temp), method = "aux", vcov = vcovHC) 

# Reject that CS FE is sufficient
temp <- qDF(unindex(moddat)) %>% tfmv(c('log_VA', 'I2E', 'E2R'), fwithin, g = cs, apply = FALSE) %>% 
...
phtest(log_VA ~ I2E + L1.I2E + L2.I2E + E2R + L1.E2R + L2.E2R, method = "aux", vcov = vcovHC,
       data = moddat %>% tfmv(c('log_VA', 'I2E', 'E2R'), fdiff, apply = FALSE) %>%
         tfmv(c("log_VA", "I2E", "E2R"), flag, n = 1:2, apply = FALSE) %>% unindex() %>%
         qDF %>% pdata.frame(index = c("cy", "Sector")))


# Reject that CS and CY FE is sufficient
temp <- qDF(unindex(moddat)) %>% tfmv(c('log_VA', 'I2E', 'E2R'), fhdwithin, 
...
summary(FE_mod)
# Residual ACF
findex(moddat)[FE_mod$obs_selection$obsRemoved, ] %$% psacf(resid(FE_mod), cs, Year) 
test_mod <- findex(moddat)[FE_mod$obs_selection$obsRemoved, ] %$% lm(resid(FE_mod) ~ L(resid(FE_mod), 1, cs, Year)) 
...
# Residual ACF
findex(moddat)[FE_mod2$obs_selection$obsRemoved, ] %$% psacf(resid(FE_mod2), cs, Year) 
findex(moddat)[FE_mod2$obs_selection$obsRemoved, ] %$% lm(resid(FE_mod2) ~ L(resid(FE_mod2), 1, cs, Year)) %>% summary

# FD Model
FD_mod <- lm(log_VA ~ L(I2E, 0:2) + L(E2R, 0:2), data = D(moddat, stubs = FALSE))
findex(moddat)[-FD_mod$na.action, ] %$% psacf.default(resid(FD_mod), cs, Year) 
findex(moddat)[-FD_mod$na.action, ] %$% lm(unattrib(resid(FD_mod)) ~ flag.default(unattrib(resid(FD_mod)), 1, cs, Year)) %>% summary
# Same as this test:
pwfdtest(log_VA ~ L(I2E, 0:1) + L(E2R, 0:1), to_plm(moddat), h0 = "fd")
# This tests against rho_FD != -0.5
pwfdtest(log_VA ~ L(I2E, 0:1) + L(E2R, 0:1), to_plm(moddat), h0 = "fe")
# Tests autocorrelation of residuals of fixed-effects model not equal to -1/(11-1) = -0.1, Wooldridge sec. 10.5.4
pwartest(log_VA ~ L(I2E, 0:1) + L(E2R, 0:1), to_plm(moddat))
# Same
FD_mod <- plm(log_VA ~ L(I2E, 0:2) + L(E2R, 0:2), data = to_plm(moddat), model = "fd")
coef(FD_mod)
...
FD_mod <- feols(formula(FD_mod), data = unindex(D(moddat, cols = vars, stubs = FALSE)), cluster = "cs")  
...
FD_mod_log <- feols(formula(FD_mod_log), unindex(D(moddat, cols = vars_log, stubs = FALSE)) %>% frename(toupper, cols = vars_log[-1]), 
                    cluster = "cs")  

FD_mod_logs <- feols(formula(FD_mod_log), moddat %>% tfmv(vars[-1], log) %>% D(cols = vars, stub = FALSE) %>% unindex(), 
                     cluster = "cs")  

FD_mod_log_r <- lmrob(log_VA ~ I2E + L1.I2E + L2.I2E + E2R + L1.E2R + L2.E2R, 
                 data = D(moddat, cols = vars_log, stubs = FALSE) %>% frename(toupper, cols = vars_log[-1]) %>% unindex())
                 # k.max = 10000, refine.tol = 1e-16)

FD_mod_logs_r <- lmrob(log_VA ~ I2E + L1.I2E + L2.I2E + E2R + L1.E2R + L2.E2R, 
                       data = moddat %>% tfmv(vars[-1], log) %>% D(cols = vars, stub = FALSE) %>% unindex())
  
FE_mod <- feols(log_VA ~ I2E + L1.I2E + L2.I2E + E2R + L1.E2R + L2.E2R, data = unindex(moddat), 
...
# Manufacturing -------------------------------------------------
moddat_MAN <- moddat %>% sbt(Sector %in% MAN) %>% qDF %>% fdroplevels %>% unindex() %>% 
              pdata.frame(index = c("cs", "Year"))
modlist_MAN <- list()
